...
export default class BassedAgeViews extends Component {
    state = {
        dataReal: [
            {
                value: 67,
    ...
    componentDidMount() {
        ...
        const gender = participants.map(item => item.viewsParticipants.items.map(item => JSON.parse(item.engageData) && JSON.parse(item.engageData).aboutThePersonality.gender))
    ...
    componentWillReceiveProps(nextProps) {
        const { action, showViewsAfterToPay } = nextProps
        this.setState({
            dataFake: [
                {
                    value: 67,
                    label: 'Female',
                    color: showViewsAfterToPay || action ? activeColors.two : disableColors.two
                }, {
                    value: 21,
                    label: 'Male',
                    color: showViewsAfterToPay || action ? activeColors.one : disableColors.one
                }
    ...
    render() {
        ...
        const { showViewsAfterToPay, action } = this.props
        return (
            showViewsAfterToPay
                ...
                : <View style={{ justifyContent: 'center', alignItems: 'center', height: "100%" }}>
                    ...
                    <View style={{ flex: 0.3, justifyContent: 'center', alignItems: 'flex-end', flexDirection: 'row', width: "100%" }}>
                        <Button iconLeft small transparent disabled>
                            <Icon name={icons.icon.male} type={icons.type} style={{ color: showViewsAfterToPay || action ? activeColors.one : disableColors.one }} />
                            <Text allowFontScaling={false} style={{ left: -10, color: showViewsAfterToPay || action ? activeColors.one : disableColors.one }}>21</Text>
                        </Button>
                        <Button iconLeft small transparent disabled>
                            <Icon name={icons.icon.female} type={icons.type} style={{ color: showViewsAfterToPay || action ? activeColors.two : disableColors.two }} />
                            <Text allowFontScaling={false} style={{ left: -10, color: showViewsAfterToPay || action ? activeColors.two : disableColors.two }}>67</Text>
                        </Button>
                    </View>

...
export default class BasedLocationShare extends Component {
    ...
    componentDidMount() {
        ...
        const topLocation = participants.map(item => item.shareParticipants.items.map(item => JSON.parse(item.engageData) && JSON.parse(item.engageData).aboutThePersonality.location.country))

...
export default class BasedAgeLikes extends Component {
    state = {
        dataReal: [{
            seriesName: 'ageRange',
            color: "#9E9E9E",
    ...
    componentDidMount() {
        ...
        const age = participants.map(item => item.likesToParticipants.items.map(item => JSON.parse(item.engageData) && (new Date().getFullYear() - new Date(JSON.parse(item.engageData).aboutThePersonality.birthDate).getFullYear())))

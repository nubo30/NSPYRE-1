...
export default class BasedAgeViews extends Component {
    state = {
        dataReal: [{
            seriesName: 'ageRange',
            color: "#9E9E9E",
    ...
    componentDidMount() {
        ...
        const age = participants.map(item => item.viewsParticipants.items.map(item => JSON.parse(item.engageData) && (new Date().getFullYear() - new Date(JSON.parse(item.engageData).aboutThePersonality.birthDate).getFullYear())))
    ...
    render() {
        ...
        const { showViewsAfterToPay } = this.props
        return (
            <View style={{ justifyContent: 'center', alignItems: 'center', height: "100%", left: 15 }}>
                <PureChart data={showViewsAfterToPay ? dataReal : dataFake} type="bar" height={150} />
                {!showViewsAfterToPay && <View style={{ justifyContent: 'center', alignItems: 'center', height: "100%", position: 'absolute', width: "100%" }} />}
            </View>
        );
    }
}
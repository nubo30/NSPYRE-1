...
export default class BassedGenderComments extends Component {
    ...
    componentDidMount() {
        ...
        const gender = participants.map(item => item.commentsToParticipants.items.map(item => JSON.parse(item.engageData) && JSON.parse(item.engageData).aboutThePersonality.gender))
    ...
    componentWillReceiveProps(nextProps) {
        const { action, showCommentsAfterToPay } = nextProps
        this.setState({
            dataFake: [
                {
                    value: 42,
                    label: 'Female',
                    color: showCommentsAfterToPay || action ? activeColors.two : disableColors.two
                }, {
                    value: 59,
                    label: 'Male',
                    color: showCommentsAfterToPay || action ? activeColors.one : disableColors.one
                }
    ...
    render() {
        ...
        const { action, showCommentsAfterToPay } = this.props
        return (
            showCommentsAfterToPay
                ...
                : <View style={{ justifyContent: 'center', alignItems: 'center', height: "100%" }}>
                    ...
                    <View style={{ flex: 0.3, justifyContent: 'center', alignItems: 'flex-end', flexDirection: 'row', width: "100%" }}>
                        <Button iconLeft small transparent disabled>
                            <Icon name={icons.icon.male} type={icons.type} style={{ color: showCommentsAfterToPay || action ? activeColors.one : disableColors.one }} />
                            <Text allowFontScaling={false} style={{ left: -10, color: showCommentsAfterToPay || action ? activeColors.one : disableColors.one }}>59</Text>
                        </Button>
                        <Button iconLeft small transparent disabled>
                            <Icon name={icons.icon.female} type={icons.type} style={{ color: showCommentsAfterToPay || action ? activeColors.two : disableColors.two }} />
                            <Text allowFontScaling={false} style={{ left: -10, color: showCommentsAfterToPay || action ? activeColors.two : disableColors.two }}>42</Text>
                        </Button>
                    </View>

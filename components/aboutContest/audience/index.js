...
import { Modal, Platform, AsyncStorage } from 'react-native'
...
export default class Audience extends Component {
    constructor() {
        ...
        this.state = {
            ...
            // Actions
            swiperIndexOurAudience: 0,
            isFirstTime: true,
        }
        this._bootstrapAsyncAudience()
    }

    _bootstrapAsyncAudience = async () => {
        AsyncStorage.getItem('@ISFIRSTTIMEAUDIENCEOPEN', async (err, result) => {
            if (result === null) {
                this.setState({ swiperIndexOurAudience: 0 })
            } else {
                this.setState({ swiperIndexOurAudience: 2 })
            }
        });
        AsyncStorage.setItem('@ISFIRSTTIMEAUDIENCEOPEN', JSON.stringify({ "value": "true" }));
    }
                                ...
                                <Content contentContainerStyle={{ flex: 1 }} scrollEnabled={false}>
                                    <Swiper
                                        ...
                                        <FormAudience
                                            // Data
                                            contest={contest}
                                            swiperIndexOurAudience={swiperIndexOurAudience}

...
class CaseOfSuccess extends Component {
    state = {
        isReady: false,
        input: "",
        refreshing: false,
    ...
    _onRefresh = () => {
        ...
        this._getListCaseOfSuccess().then(() => {
            this.setState({ refreshing: false });
        });
    }

    componentDidMount() {
        this._getListCaseOfSuccess()
    }

    _getListCaseOfSuccess = async () => {
        try {
            const response = await API.graphql(graphqlOperation(queries.listCaseOfSuccess, { params: "" }))
            this.setState({ listCaseOfSucess: JSON.parse(response.data.listCaseOfSuccess) })
        } catch (error) {
            console.log(error)
        }
    }
    ...
    render() {
        ...
        return (
            <Container>
                ...
                {listCaseOfSucess !== null
                    ? listCaseOfSucess.length ?
                        filterCaseOfSuccess.length
                            ? <FlatList
                                data={filterCaseOfSuccess}
                                                        ...
                                                        <View style={{ backgroundColor: colorsPalette.primaryShadowColor, width: "100%", height: "100%", borderRadius: 5, alignItems: 'center', justifyContent: 'space-evenly' }}>
                                                            ...
                                                            <View style={{ width: '100%', flexDirection: 'row', justifyContent: 'center' }}>
                                                                <Text
                                                                    minimumFontScale={wp(4)}
                                                                    allowFontScaling={false}
                                                                    style={{ color: colorsPalette.secondaryColor, fontSize: wp(6) }}>
                        ...
                        : <View style={{ flex: 1, alignItems: 'center' }}><Text allowFontScaling={false} style={{ top: 30, alignSelf: 'center', textAlign: 'center', width: "90%", fontSize: wp(6), color: colorsPalette.gradientGray }}>Ooh! Apparently there are no success case, but this will not last long, some will appear!</Text></View>
                    : <PlaceholderAll />
                }
            </Container>
        );
...
export default withNavigation(CaseOfSuccess)